...
class RetinexNet(nn.Module):
    ...
    def forward(self, input_low, input_high):
        ...
        # Forward RelightNet
        R_delta, I_delta, _ , _ , _, _, _, _, _, _, _, _, _ = self.RelightNet(I_low, R_low)
        ...
        self.loss_Relight = self.relight_loss + 3 * self.Ismooth_loss_delta + self.vgg_LDM(R_delta * I_delta_3, input_high)
    ...
    def train(self,
        ...
        for epoch in range(start_epoch, epoch):
            ...
            for batch_id in range(start_step, numBatch):
                ...
                iter_num += 1
                #최고의 loss를 가지는 weight를 저장하는 파일 따로 만들기
                if loss < best_loss:
                    best_loss = loss
